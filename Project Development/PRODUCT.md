...
- Name resolution climbs a bounded ladder: memory, the project's own domain, package registries, the shape of a multi-word name, then registry search. Every lap ends at the same identity gate, and the whole ladder is capped at 40 requests. Where a name resolved before is remembered for 30 days and clearable with `forget_resolution`.
...
- 536 passing offline unit tests (`tests/`), 48 skipped behind opt-in gates (live network, Postgres, browser rendering). Verified by a full run, not asserted.
